...
module.exports.controller = function (app) {
 ...
 /**
  ...
  app.post('/account/delete', function (req, res, next) {
    User.model().remove({ _id: req.user._id }, function (err) {
      if (err) {
        return next(err);
      }
      req.logout();
 ...
 /**
  ...
  app.get('/account/link/facebook/callback', function (req, res, next) {
    Passport.authenticate('facebook', {
      ...
      User.model().findOne({ facebook: info.profile._json.id }, function (err, existingUser) {
        if (existingUser) {
  ...
  app.get('/account/link/twitter/callback', function (req, res, next) {
    Passport.authenticate('twitter', {
      ...
      // Let's check to make sure we don't already have an account with the same credentials
      User.model().findOne({ twitter: info.profile._json.id }, function (err, existingUser) {
        if (existingUser) {
  ...
  app.get('/account/link/github/callback', function (req, res, next) {
    Passport.authenticate('github', {
      ...
      // Let's check to make sure we don't already have an account with the same credentials
      User.model().findOne({ github: info.profile._json.id }, function (err, existingUser) {
        if (existingUser) {
  ...
  app.get('/account/link/google/callback', function (req, res, next) {
    Passport.authenticate('google', {
      ...
      // Let's check to make sure we don't already have an account with the same credentials
      User.model().findOne({ google: info.profile._json.id }, function (err, existingUser) {
        if (existingUser) {
